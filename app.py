...
# 音圧ヒートマップと等高線の計算（ホーンスピーカー指向性モデル）
def calculate_heatmap_and_contours(speakers, L0, r_max, grid_lat, grid_lon):
    ...
    for spk in speakers:
        ...
        for direction in dirs:
            angle_diff = np.abs(np.degrees(np.arctan2(grid_coords[:, 1] - lon, grid_coords[:, 0] - lat)) - direction) % 360
            angle_diff = np.minimum(angle_diff, 360 - angle_diff)
            directivity_factor = np.clip(1 - angle_diff / 60, 0, 1)  # 減衰範囲を60度に設定
